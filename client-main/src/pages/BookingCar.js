...
function BookingCar({ match }) {
  ...
  useEffect(() => {
    setTotalAmount((prevTotalAmount) => {
      const rentAmount = totalHours * data.rentPerHour;
      let newTotalAmount = rentAmount;

      if (driver) {
        newTotalAmount += totalHours * 30;
      }

      return newTotalAmount;
    });
  }, [driver, totalHours, data.rentPerHour]);
  ...
  return (
    <form>
      ...
      <div style={{ color: '#3498DB' }}>
        ...
        <button className='btn1' onClick={BookingCar} >handle Book Now</button>
      </div>
    </form>
  )
}
